...
class BidController extends Controller
{
    ...
    {
        $parameter = $id;
        $cek_jumlahpengajuan = ApproveBid::leftJoin('bid_user', 'bid_user.id_biduser', '=', 'approve_bid.bid_user_id')
            ->leftJoin('bid', 'bid.id_bid', '=', 'approve_bid.bid_id')
    ...
    public function approve_bid(Request $request)
    {
        ...
        $cek_jumlahpengajuan = ApproveBid::leftJoin('bid_user', 'bid_user.id_biduser', '=', 'approve_bid.bid_user_id')
            ->leftJoin('bid', 'bid.id_bid', '=', 'approve_bid.bid_id')
            ->where('approve_bid.bid_id', $request->bid_id)
            ->where('approve_bid.status_bid', 1)
            ->sum('approve_bid.permintaan_kirim');
        $get_tanggal_po = Bid::where('id_bid', $request->bid_id)->first();
        $kuota_sisa = ($get_tanggal_po->jumlah + $get_tanggal_po->add_kuota) - ($cek_jumlahpengajuan * 8000);
        if ($kuota_sisa <= 0) {
            Alert::error('Error', 'Kuota Lelang Anda habis.', 3000);
            return redirect()->back()->with('Error', 'Kuota Lelang Anda habis', 3000);
        }
...
_Sent Via *PT SURYA PANGAN SEMESTA NGAWI*_",
            ...
            $notif->save();
            Alert::success('Berhasil', 'Data anda berhasil di Simpan.', 3000);
            return redirect()->back()->with('success', 'Data anda berhasil di Simpan.', 3000);
            // die();
        } elseif ($data->status_biduser == 1) {

            Alert::success('Berhasil', 'Data Approve Anda Sudah Ada.', 3000);
            return redirect()->back()->with('success', 'Data Approve Anda Sudah Ada.', 3000);
        }
        Alert::success('Berhasil', 'Data anda berhasil di Simpan.', 3000);
        return redirect()->back()->with('success', 'Data anda berhasil di Simpan.', 3000);
    }
