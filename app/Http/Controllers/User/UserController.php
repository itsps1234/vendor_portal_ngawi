...
class UserController extends Controller
{
    ...
    function create(Request $request)
    {
        ...
        $moveNPWP = $fileNPWP->move(
            'public/img/npwp/profile_user',
            $imageNameNPWP
        );
        ...
        $movePI = $filePI->move(
            'public/img/pakta_integritas/profile_user',
            $imageNamePI
        );
        ...
        $cek_username = DB::table('users')
            ...
            ->first();
        $cek_npwp = DB::table('users')
            ->where('npwp', $request->npwp)
            ->first();
        $cek_ktp = DB::table('users')
            ->where('ktp', $request->ktp)
            ->first();
        $cek_address1 = DB::table('districts')
            ->where('id', $request->id_kecamatannpwp)
            ->first();
        $cek_address2 = DB::table('regencies')
            ->where('id', $request->id_kabupatennpwp)
            ->first();
        $cek_address3 = DB::table('provinces')
            ->where('id', $request->id_provinsinpwp)
            ->first();

        $nomer_urut = DB::table('users')
            ->where('GroupCode', '1PBB')
            ->count();
        ...
        if (($cek_email == '') | ($cek_username == '')) {
            ...
            if ($query) {
                ...
                Mail::send(
                    'dashboard.user.emailVerificationEmail',
                    ['token' => $token],
                ...
                return redirect()
                    ->back()
                    ->with('fail', 'Data yang anda masukan salah');
            }
        } else {
            return redirect()
                ->back()
                ->with(
                    'fail',
                    'Data email, Username, npwp, dan ktp anda masukan sudah ada'
                );
        }
    }
    public function verifyAccount($token)
    {
        ...
        Alert::success(
            ' Selamat ',
            ' Silahkan Tunggu Validasi Dari Admin 1x24 Jam'
        );
        return redirect()
            ->route('user.login')
            ->with('Selamat', $message);
    }
    ...
    public function getkabupaten(Request $request)
    {
        ...
        $kabupaten = Regency::where('province_id', $id_provinsi)
            ->orderBy('name', 'ASC')
            ->get();
    ...
    public function getkecamatan(Request $request)
    {
        ...
        $kecamatan = District::where('regency_id', $id_kabupaten)
            ->orderBy('name', 'ASC')
            ->get();
    ...
    public function getcount_notifikasi(Request $request)
    {
        ...
        $getcount_pajak = DB::table('potong_pajak')
            ...
            ->count();
        $notif_lelang = Bid::where('lokasi', 'NGAWI')
            ->where('bid_status', '1')
            ->orderBy('date_bid', 'DESC')
            ->count();
        $result = [
            'getcount_transaksi' => $getcount_transaksi,
            'getcount_broadcast' => $getcount_broadcast,
            'getcount_pajak' => $getcount_pajak,
            'notif_lelang' => $notif_lelang,
        ];
    ...
    public function getdesa(Request $request)
    {
        ...
        $desa = Village::where('district_id', $id_kecamatan)
            ->orderBy('name', 'ASC')
            ->get();
    ...
    public function home()
    {
        ...
        $query = DB::table('bid')
            ->where('batas_bid', '<=', $date)
            ->where('bid_status', '1')
            ->orderBy('date_bid', 'DESC')
            ->get();
        foreach ($query as $kediri) {
            $data = DB::table('bid')
                ->where('id_bid', '=', $kediri->id_bid)
                ->update([
                    'bid_status' => '0',
        ...
        $site_other = DB::table('bid')
            ...
            ->limit(5)
            ->get();
        $site_ngawi_ciherang = DB::table('bid')
            ->where('lokasi', 'NGAWI')
            ->where('name_bid', 'GABAH BASAH CIHERANG')
            ->orderBy('date_bid', 'DESC')
            ->limit(5)
            ->get();
        $site_ngawi_pandanwangi = DB::table('bid')
            ->where('lokasi', 'NGAWI')
            ->where('name_bid', 'GABAH BASAH PANDAN WANGI')
            ->orderBy('date_bid', 'DESC')
            ->limit(5)
            ->get();
        $site_ngawi_ketanputih = DB::table('bid')
            ->where('lokasi', 'NGAWI')
            ->where('name_bid', 'GABAH BASAH KETAN PUTIH')
            ->orderBy('date_bid', 'DESC')
            ->limit(5)
            ->get();
        $site_ngawi = DB::table('bid')
            ->where('lokasi', 'NGAWI')
            ->where('bid_status', '1')
            ->orderBy('date_bid', 'DESC')
            ->get();
        $lainnya = Bid::orderBy('id_bid', 'desc')
            ->limit(4)
            ->get();
        $berita = news::get();
        $get_site_ngawi_longgrain = DB::table('bid')
            ->where('lokasi', 'NGAWI')
        ...
        if ($get_site_ngawi_longgrain == '') {
            $site_ngawi_longgrain = DB::table('bid')
                ->where('lokasi', 'NGAWI')
                ->where('name_bid', 'GABAH BASAH LONG GRAIN')
                ->where('bid_status', '1')
                ->orderBy('id_bid', 'DESC')
        ...
        if (Auth()->user()) {
            $id = Auth::user()->id;
            $count_transaksi = DataPO::leftjoin(
                'bid',
                'bid.id_bid',
                '=',
                'data_po.bid_id'
        ...
        return view('dashboard.user.home', [
            ...
            'site_ngawi' => $site_ngawi,
            'site_ngawi_longgrain' => $site_ngawi_longgrain,
            'site_ngawi_ciherang' => $site_ngawi_ciherang,
            'site_ngawi_pandanwangi' => $site_ngawi_pandanwangi,
            'site_ngawi_ketanputih' => $site_ngawi_ketanputih,
    ...
    public function lelang_detail($id)
    {
        if (Auth()->user()) {
            $data = DB::table('bid')
                ->where('id_bid', $id)
                ->first();
            $user_bid = DB::table('bid_user')
                ->join('users', 'users.id', 'bid_user.id_biduser')
                ->get();
            $bid_user = DB::table('bid_user')
                ->where('user_id', Auth()->user()->id)
                ->where('bid_id', $id)
                ->first();
            $partisipasi_bid = DB::table('bid_user')
                ->join('users', 'users.id', '=', 'bid_user.user_id')
                ->where('bid_id', $id)
                ->get();
            ...
            return view('dashboard.user.daftar_lelang.detail_lelang', [
                ...
                'jumlah_kuotatruk' => $jumlah_kuotatruk,
                'sisakuota' => $sisakuota,
                'get_sisakuota' => $get_sisakuota,
            ]);
        } else {
            return view('dashboard.user.login');
        }
    ...
    public function pangan_pertanian()
    {
        $berita = DB::table('news')
            ->limit(4)
            ->get();
        $berita_pangan = DB::table('news')
            ->where('kategori', 'pangan_pertanian')
            ->get();
        return view('dashboard.user.berita.dt_panganpertanian', [
            'berita' => $berita,
            'berita_pangan' => $berita_pangan,
        ]);
    }
    ...
    public function teknologi_inovasi()
    {
        $berita = DB::table('news')
            ...
            ->get();
        $berita_pangan = DB::table('news')
            ->where('kategori', 'teknologi_inovasi')
            ->get();
        return view('dashboard.user.berita.dt_teknologiinovasi', [
            'berita' => $berita,
            'berita_pangan' => $berita_pangan,
        ]);
    }
    ...
    public function ekonomi_perdagangan()
    {
        $berita = DB::table('news')
            ...
            ->get();
        $berita_pangan = DB::table('news')
            ->where('kategori', 'ekonomi_perdagangan')
            ->get();
        return view('dashboard.user.berita.dt_ekonomiperdagangan', [
            'berita' => $berita,
            'berita_pangan' => $berita_pangan,
        ]);
    }
    ...
    public function international()
    {
        $berita = DB::table('news')
            ...
            ->get();
        $berita_pangan = DB::table('news')
            ->where('kategori', 'international')
            ->get();
        return view('dashboard.user.berita.dt_international', [
            'berita' => $berita,
            'berita_pangan' => $berita_pangan,
        ]);
    }
    ...
    public function search(Request $request)
    {
        ...
        return view('dashboard.user.berita.dt_search', [
            'search' => $search,
            'berita' => $berita,
        ]);
    }
    ...
    public function populer()
    {
        ...
        $berita = DB::table('news')
            ->limit(4)
            ->get();
        return view('dashboard.user.berita.populer.dt_populer', [
            'populer' => $populer,
            'berita' => $berita,
        ]);
    }
    ...
    public function terbaru()
    {
        $berita = DB::table('news')
            ...
            ->paginate(3);
        return view('dashboard.user.berita.terbaru.dt_terbaru', [
            'berita' => $berita,
        ]);
    }
    ...
    {
        $detail_berita = DB::table('news')
            ->where('id_news', $id)
            ->first();
        $berita = DB::table('news')
            ->limit(4)
            ->get();
        return view('dashboard.user.berita.dt_detailberita', [
    ...
    public function daftar_lelang()
    {
        ...
        $site_other = DB::table('bid')
            ...
            ->orderBy('date_bid', 'DESC')
            ->get();
        $get_site_ngawi_ciherang = DB::table('bid')
            ->where('lokasi', 'NGAWI')
            ->where('name_bid', 'GABAH BASAH CIHERANG')
            ->where('bid_status', '1')
            ->orderBy('id_bid', 'ASC')
        ...
        return Datatables::of(
            ...
            ->addColumn('date_bid', function ($list) {
                ...
                return date('Y-m-d', strtotime($result)) .
                    '<br><span class="btn-info">Open Lelang 08:00</span>';
            })
            ->addColumn('waktu_pengajuan', function ($list) {
                $result = $list->date_biduser;
                return date('Y-m-d', strtotime($result)) .
                    '<br><span class="btn-success">' .
            ...
            ->addColumn('batas_po', function ($list) {
                ...
                return date('Y-m-d', strtotime($result)) .
                    '<br><span class="btn-warning">Batas 12:00</span>';
            })
            ...
            ->addColumn('jumlah_disetujui', function ($list) {
                ...
                return '<a id="btn_list_po" style="margin:2px;" href="' .
                    route('user.data_list_po', ['id' => $list->id_biduser]) .
                    '" name="' .
                    $list->id_approvebid .
                    '" title="Lihat PO" class="lihat_po btn btn-info m-btn m-btn--icon btn-sm m-btn--icon-only">
                <i class="" style="color:white;"> </i> ' .
                    $list->permintaan_kirim .
                    ' Truk 
                </a>';
            })
            ...
            ->addColumn('status_biduser', function ($list) {
                if ($list->status_biduser == 1) {
                    return '<a style="margin:2px;background-color:#04B431" id="btn_disetujui" name="' .
                        $list->user_id .
                        '" data-jumlahkirim="' .
                        $list->jumlah_kirim .
                        '" data-idnyabid="' .
                        $list->id_bid .
                        '" title="Disetujui" class=" btn m-btn m-btn--icon btn-sm m-btn--icon-only">
                ...
                <i class="" style="color:white;"> Ditolak </i>
                    ...
                    return '<a style="margin:2px;background-color:yellow" name="' .
                        $list->id_biduser .
                        '" title="Pengiriman Telat" class="btn m-btn m-btn--icon btn-sm m-btn--icon-only">
    ...
    public function detail_pengajuan($id)
    {
        ...
        $data = DB::table('approve_bid')
            ...
            ->select(
                'approve_bid.*',
                'bid_user.*',
                DB::raw('SUM(approve_bid.permintaan_kirim) AS siap_kirim'),
                'approve_bid.bid_id AS idnya_bid'
            )
            ->where('approve_bid.bid_id', $id)
    ...
    public function lihat_po($id)
    {
        ...
        $data = DataPO::join(
            'approve_bid',
            'data_po.id_approvebid',
            '=',
            'approve_bid.id_approvebid'
        )
    ...
    public function transaksi_detail($id)
    {
        $data = DB::table('transaksi')
            ->join(
                ...
                'bid_user.id_biduser',
                '=',
                'transaksi.id_biduser_id'
            )
            ->join('users', 'users.id', '=', 'transaksi.id_vendor_transaksi')
            ->join('bid', 'id_bid', '=', 'transaksi.id_bid_transaksi')
            ->where('id_biduser_id', $id)
            ->first();
        return view('dashboard.user.transaksi.dt_transaction_detail', [
            'data' => $data,
        ]);
    }
    ...
    public function data_list_po($id)
    {
        $data = DataPO::join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
            ...
            ->selectRaw(
                'data_po.*,users.nama_vendor,penerimaan_po.*,lab2_gb.aksi_harga_gb,lab2_gb.harga_akhir_gb,bid.name_bid,bid.open_po,bid.batas_bid,bid_user.id_biduser,bid_user.jumlah_kirim'
            )
    ...
    {
        $data = DB::table('gabahincoming_qc')
            ->where('gabahincoming_kode_po', $id)
            ->first();
        return json_encode($data);
    }
    ...
    public function konfirmasi_bongkar(Request $request)
    {
        ...
        if ($request->bongkar == 'bongkar') {
            ...
            $data1 = DataPO::where('id_data_po', $request->id_datapo)->update([
                'status_bid' => '7',
            ]);
            $data2 = PenerimaanPO::where(
            ...
            $log->aktivitas_user =
                'Konfirmasi Supplier, setuju bongkar. Kode PO: ' .
                $data->lab1_kode_po_gb .
                ' Plan Harga : ' .
                $data->plan_harga_gb;
            $log->keterangan_aktivitas = 'Selesai';
            $log->created_at = date('Y-m-d H:i:s');
            ...
            $log->id_objek_aktivitas_user = $data->id_lab1_gb;
            $log->aktivitas_user =
                'Konfirmasi Supplier, Tolak bongkar. Kode PO: ' .
                $data->lab1_kode_po_gb .
                ' Plan Harga : ' .
                $data->plan_harga_gb;
            $log->keterangan_aktivitas = 'Selesai';
        ...
        return redirect()
            ->back()
            ->with('Berhasil', 'Data anda berhasil di Simpan');
    }
    ...
    public function akun()
    {
        ...
        // dd($prov);
        return view(
            'dashboard.user.akun.dt_account',
    ...
    public function update_akun(Request $request)
    {
        ...
        if (
            $request->file_paktaintegritas == null &&
            $request->file_fis == null
        ) {
            ...
            if ($data->fis != '' && $data->fis != null) {
                $path =
                    '/home/sumb1497/public_html/bid.com/public/img/fis/profile_user/';
            ...
            $moveKTP = $filefis->move(
                'public/img/fis/profile_user',
                $imageNamefis
            );
            $data->username = $request->username;
            $data->nama_vendor = $request->nama_vendor;
            ...
            if (
                ...
                $path =
                    '/home/sumb1497/public_html/bid.com/public/img/pakta_integritas/profile_user/';
            ...
            $imageNamePakta =
                time() . '.' . $request->file_paktaintegritas->extension();
            $moveKTP = $filePakta->move(
                'public/img/pakta_integritas/profile_user',
                $imageNamePakta
            );
            $data->username = $request->username;
            ...
            if (
                ...
                $path =
                    '/home/sumb1497/public_html/ngawi.suryapangansemesta.store/public/img/pakta_integritas/profile_user/';
                $path1 =
                    '/home/sumb1497/public_html/ngawi.suryapangansemesta.store/public/img/fis/profile_user/';
            ...
            $imageNamePakta =
                time() . '.' . $request->file_paktaintegritas->extension();
            $movePakta = $filePakta->move(
                'public/img/pakta_integritas/profile_user',
                $imageNamePakta
            );
            $filefis = $request->file('file_fis');
            $imageNamefis = time() . '.' . $request->file_fis->extension();
            $moveFIS = $filefis->move(
                'public/img/fis/profile_user',
                $imageNamefis
            );
            $data->username = $request->username;
    ...
    public function update_ktp(Request $request)
    {
        // dd($request->all());
        $cek_address1 = DB::table('districts')
            ->where('id', $request->id_kecamatanktp)
            ->first();
        $cek_address2 = DB::table('regencies')
            ->where('id', $request->id_kabupatenktp)
            ->first();
        $cek_address3 = DB::table('provinces')
            ->where('id', $request->id_provinsiktp)
            ->first();
        $cek_address4 = DB::table('villages')
            ->where('id', $request->id_desaktp)
            ->first();
        $data = User::where('id', $request->id)->first();
        if ($request->file_ktp == null) {
            ...
            $data->address1 =
                $request->keterangan_alamat_ktp .
                ' RT. ' .
                $request->rt_ktp .
                ' / RW. ' .
                $request->rw_ktp;
            $data->address2 =
                'KEL ' . $cek_address4->name . ' KEC. ' . $cek_address1->name;
            ...
            if ($data->gambar_ktp != '' && $data->gambar_ktp != null) {
                $path =
                    '/home/sumb1497/public_html/bid.com/public/img/ktp/profile_user/';
            ...
            $moveKTP = $fileKTP->move(
                'public/img/ktp/profile_user',
                $imageNameKTP
            );
            $data->nama_ktp = $request->nama_ktp;
            $data->ktp = $request->ktp;
    ...
    public function update_npwp(Request $request)
    {
        ...
        if ($request->file_npwp == null) {
            ...
            $data->SPS_AlamatNPWP_c =
                $request->keterangan_alamat_npwp .
                ' RT. ' .
                $request->rt_npwp .
                ' RW. ' .
                $request->rw_npwp .
            ...
            if ($data->gambar_npwp != '' && $data->gambar_npwp != null) {
                $path =
                    '/home/sumb1497/public_html/bid.com/public/img/npwp/profile_user/';
    ...
    public function pemberitahuan()
    {
        ...
        if (
            ...
            $firsttabs = isset($broadcaster1->id_broadcast)
                ? $broadcaster1->id_broadcast
                : $broadcaster->first()->id_broadcast;
        }
        $data = DB::table('broadcast')
            ->orderBy('id_broadcast', 'desc')
            ->get();
        return view('dashboard.user.pemberitahuan.dt_pemberitahuan', [
...
Berlaku 4 Menit",
            ...
            if (
                ...
                if (
                    ...
                    return redirect()
                        ->route('user.new_user')
                        ->with('Mohon Maaf', 'Akun Anda Tidak Aktif');
                } elseif (auth()->user()->status_user == 2) {
                    Auth::logout();
                    Alert::warning('Mohon Maaf', 'Akun Anda Tidak Aktif');
                    return redirect()
                        ->route('user.new_user')
    ...
    public function login_check(Request $request)
    {
        // return $request->all();
        if (Auth()->user()) {
            $response = ['status' => 200, 'message' => 'Anda Masih Berstatus Login'];
            ...
            if (is_numeric($request->username)) {
                if (
                    ...
                    if (
                        ...
                        $response = ['status' => 200, 'user' => Auth::user(),   'message' => 'Successfully Login! Welcome Back'];
                        return response()->json($response);
                    } elseif (
                        auth()->user()->is_email_verified == 0 &&
                        auth()->user()->status_user == 0
                        ...
                        Auth::logout();
                        $response = ['status' => 400, 'message' => 'Mohon Cek Email Anda Sekarang Dan Lakukan Verifikasi'];
                        return response()->json($response);
                    } elseif (auth()->user()->status_user == 0) {
                        Auth::logout();
                        $response = ['status' => 400, 'message' => 'Akun Anda Tidak Aktif'];
                        return response()->json($response);
                    } else {
                        Auth::logout();
                        $response = ['status' => 400, 'message' => 'Akun Anda Dalam Proses Validasi'];
                        return response()->json($response);
                    }
                } else {
                    $response = ['status' => 400, 'message' => 'Mohon Masukkan Email Atau Username Dan Password Dengan Benar'];
                    return response()->json($response);
                }
            } elseif (filter_var($request->username, FILTER_VALIDATE_EMAIL)) {
                if (
        ...
        // $user = User::where('nomer_hp', $request->email)->first();
        // return $user;
        // if ($user != '[]' && Hash::check($request->password, $user->password)) {
        // $token = $user->createToken('Personal Access Token')->plainTextToken;
        // $response = ['status' => 200, 'token' => $token, 'user' => $user, 'message' => 'Successfully Login! Welcome Back'];
        // $response = ['status' => 200, 'user' => $user, 'message' => 'Successfully Login! Welcome Back'];
        // return response()->json($response);
        // } else if ($user == '[]') {
    ...
    public function new_home()
    {
        ...
        $notif = DB::table('broadcast')->count();
        // dd($notif);
        return view('dashboard.user.new_user.home', [
            'site_kediri' => $site_kediri,
            'site_ngawi' => $site_ngawi,
            'site_ngawi_longgrain' => $site_ngawi_longgrain,
            'site_ngawi_ciherang' => $site_ngawi_ciherang,
    ...
    public function new_user_index(Request $request)
    {
        if (Auth()->user()) {
            return view('dashboard.user.new_user.home');
        } else {
    ...
    public function new_daftar_lelang()
    {
        $date = date('Y-m-d H:i:s');
        $query = DB::table('bid')
            ->where('batas_bid', '<=', $date)
        ...
        // dd($site_ngawi_longgrain);
        return view('dashboard.user.new_user.daftar_lelang.lelang', [
            'site_kediri' => $site_kediri,
            'site_subang' => $site_subang,
            'site_ngawi_longgrain' => $site_ngawi_longgrain,
            'site_ngawi_ciherang' => $site_ngawi_ciherang,
    ...
    public function new_lelang_detail($id)
    {
        if (Auth()->user()) {
            $data = DB::table('bid')
                ->where('id_bid', $id)
            ...
            $get_sisakuota = $jumlah_kuota - $cek_jumlahpengajuan * 8000;
            $sisakuota = $get_sisakuota / 8000;
            return view('dashboard.user.new_user.daftar_lelang.detail_lelang', [
                'data' => $data,
                'bid_user' => $bid_user,
                'partisipasi_bid' => $partisipasi_bid,
                'jumlah_kuota' => $jumlah_kuota,
    ...
    public function new_data_list_po($id)
    {
        $data = DataPO::join('bid', 'bid.id_bid', '=', 'data_po.bid_id')
            ...
            ->get();
        // dd($data);
        return view('dashboard.user.new_user.transaksi.data_list_po', [
            'data' => $data,
        ]);
    }
    ...
    public function potong_pajak()
    {
        ...
        if ($broadcaster_count > 0) {
            $firsttabs = $broadcaster->first()->id_potong_pajak;
        } else {
            $firsttabs = '';
        }
        // dd($broadcaster);
    ...
    public function new_lelang_storeuser(Request $request)
    {
        // dd($request->all());
        if (Auth()->user()) {
            $id_user = Auth()->user()->id;
            ...
            if ($get_data == '0') {
                ...
                if ($request->file('image_biduser') != null) {
                    ...
                    return redirect()->route('user.new_home');
                } else {
                    $data = new BidUser();
                    $data->user_id = $id_user;
                    $data->bid_id = $request->bid_id;
    ...
    {
        return view('dashboard.user.new_user.about.about');
    }
    public function new_update_akun(Request $request)
    {
        // dd($request->all());
        $data = User::where('id', $request->id)->first();
        $data->username = $request->username;
    ...
    function new_video_panduan()
    {
        return view('dashboard.user.new_user.about.video_panduan');
    }
    function new_logout()
    {
        Auth::guard('web')->logout();
        Alert::success('Sukses', 'Anda Berhasil Logout');
        return redirect('/user/new_user')->with(
            'Sukses',
